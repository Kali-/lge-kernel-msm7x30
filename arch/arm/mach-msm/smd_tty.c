...
#ifdef CONFIG_MACH_LGE
static DEFINE_SPINLOCK(smd_tty_spin_lock);
#endif
...
static void smd_tty_close(struct tty_struct *tty, struct file *f)
{
	...
	mutex_lock(&smd_tty_lock);
#ifdef CONFIG_MACH_LGE
	spin_lock_irqsave(&smd_tty_spin_lock, flags);
#endif
	if (--info->open_count == 0) {
		if (info->tty) {
...
#ifdef CONFIG_MACH_LGE
	spin_unlock_irqrestore(&smd_tty_spin_lock, flags);
#endif
	mutex_unlock(&smd_tty_lock);
}
...
static int smd_tty_dummy_probe(struct platform_device *pdev)
{
 ...
 // LGE_VT	
	else if (!strcmp(pdev->name, smd_ch_name[16])) // mvs
		complete_all(&smd_tty[16].ch_allocated);
	else if (!strcmp(pdev->name, smd_ch_name[17])) // csd
...
static int __init smd_tty_init(void)
{
    ...
    // LGE_VT	
    	tty_register_device(smd_tty_driver, 16, 0); // mvs
	tty_register_device(smd_tty_driver, 17, 0); // csd
    // LGE_VT		
    ...
    // LGE_VT	
    	init_completion(&smd_tty[16].ch_allocated); // mvs
	init_completion(&smd_tty[17].ch_allocated); // csd
    // LGE_VT		
	...
	if (ret)
		goto unreg0;
// LGE_VT
	smd_tty[16].driver.probe = smd_tty_dummy_probe;
	smd_tty[16].driver.driver.name = smd_ch_name[16];
	smd_tty[16].driver.driver.owner = THIS_MODULE;
	ret = platform_driver_register(&smd_tty[16].driver);
	if (ret)
		goto unreg7;
...
// LGE_VT	
	...
	if (ret)
		goto unreg17; // LGE_VT change unreg7 ==> unreg17
...
// LGE_VT
unreg17:
	platform_driver_unregister(&smd_tty[17].driver);
unreg16:
	platform_driver_unregister(&smd_tty[16].driver);	
// LGE_VT	
unreg7:
...
// LGE_VT	
    	tty_unregister_device(smd_tty_driver, 16); // mvs
	tty_unregister_device(smd_tty_driver, 17); // csd
// LGE_VT		
