...
{
#if 0
	printk(KERN_ERR "[msm_cpufreq_subsuspend]  ~~~~~~~~~~~~~\n");
	if( policy->cpu == 0 )
		return msm_cpufreq_target(policy, 741517, 0);
	else
#endif
	return 0;
}

int	msm_cpufreq_subresume(struct cpufreq_policy *policy)
{
	printk(KERN_ERR "[msm_cpufreq_subresume]  CPU [%d]~~~~~~~~~~~~~\n", policy->cpu);
