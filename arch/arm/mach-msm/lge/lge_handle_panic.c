...
#define HIDDEN_MAGIC	0x618EFFFF
...
static int hidden_reset_complete(const char *val, struct kernel_param *kp)
{
	...
	printk(KERN_INFO "[hidden_reset] hidden reset is complete %d\n", on_hidden_reset);
	
	return 0;
}
module_param_call(hidden_reset_complete, hidden_reset_complete, param_get_bool, &dummy_arg, S_IWUSR | S_IRUGO);

static int hidden_reset_enable(const char *val, struct kernel_param *kp)
{
	void *copy_addr;
	...
	if (*val == 48) {
		printk(KERN_INFO"[hidden_reset] hidden reset is disabled\n");
		*((unsigned int *)copy_addr) = HIDDEN_MAGIC;
		hreset_enable_flag = 0;
	} else {
		printk(KERN_INFO"[hidden_reset] hidden reset is enabled\n");
		*((unsigned int *)copy_addr) = fb_phys_addr;
		hreset_enable_flag = 1;
	}
...
module_param_call(hidden_reset_enable, hidden_reset_enable, param_get_bool, &dummy_arg, S_IWUSR | S_IRUGO);
