...
#if defined(LGE_VT)
            //  set dtx_mode
           	set_dtx_mode_msg.dtx_mode = cpu_to_be32(audio->dtx_mode);            
#else
...
#if defined(LGE_VT)                 
                /* Extract AMR rate type*/
                args++; 
				buf_node->frame.rate_type = be32_to_cpu(*args);                 
                pr_debug("%s: UL AMR rate_type %d\n",__func__,buf_node->frame.rate_type);
#endif                
...
#if defined(LGE_VT)
                // set AMR rate type
				dl_reply.param2 = cpu_to_be32(buf_node->frame.rate_type);
#else
...
static int audio_mvs_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
        ...
        if(audio->exit)
        {
            pr_info("audio_mvs_thread force exit\n");
            break;
        }
#endif
...
#if defined(LGE_VT)                    
                    // set exit flag
                    audio_mvs_info.exit= 0;
#endif
					file->private_data = &audio_mvs_info;
				} else {
#if defined(LGE_VT) 				
                    // set exit flag
                    audio_mvs_info.exit= 1;

                    // wakeup rpc
                    msm_rpc_read_wakeup(audio_mvs_info.rpc_endpt);
#endif                        
