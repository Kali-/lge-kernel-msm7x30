...
#define LB_DISABLE			0x00
#define LB_ENABLE			0x01
#define LB_PATH_HANDSET	0x10
#define LB_PATH_HEADSET	0x20
#define LB_PATH_SPEAKER	0x30
#define LB_IS_ENABLE(x)		(x&0x0F)

/*******************************************************************************
*  headset loopback Rx section
*
********************************************************************************/
static struct adie_codec_action_unit headset_mono_rx_8KHz_osr256_actions[] =
	HEADSET_RX_LEGACY_8000_OSR_256;

static struct adie_codec_hwsetting_entry headset_lb_rx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = headset_mono_rx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(headset_mono_rx_8KHz_osr256_actions),
	}
};

static struct adie_codec_dev_profile lb_headset_rx_profile = {
	.path_type = ADIE_CODEC_RX,
...
/*******************************************************************************
*  headset loopback Tx section
*
********************************************************************************/
static struct adie_codec_action_unit headset_mono_tx_8KHz_osr256_actions[] =
{
	{ADIE_CODEC_ACTION_STAGE_REACHED,  ADIE_CODEC_DIGITAL_OFF},
	...
	{ADIE_CODEC_ACTION_ENTRY,  ADIE_CODEC_PACK_ENTRY(0x82, 0xFF, 0x5D)},
	{ADIE_CODEC_ACTION_STAGE_REACHED,  ADIE_CODEC_DIGITAL_READY},
...
static struct adie_codec_hwsetting_entry headset_lb_tx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = headset_mono_tx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(headset_mono_tx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile lb_headset_tx_profile = {
	.path_type = ADIE_CODEC_TX,
...
static struct adie_codec_action_unit ispeaker_tx_8KHz_osr256_actions[] =
	...
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x86, 0xff, 0x00)},
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x8A, 0x50, 0x40)},
	{ADIE_CODEC_ACTION_ENTRY, ADIE_CODEC_PACK_ENTRY(0x91, 0xFF, 0x01)}, /*start loopback*/		
	{ADIE_CODEC_ACTION_STAGE_REACHED, ADIE_CODEC_DIGITAL_ANALOG_READY}, \
...
static struct adie_codec_dev_profile lb_speaker_tx_profile = {
	.path_type = ADIE_CODEC_TX,
...
/*******************************************************************************
*  handset loopback Rx section
*
********************************************************************************/
static struct adie_codec_action_unit handset_rx_8KHz_osr256_actions[] =
		HANDSET_RX_8000_OSR_256;
...
static struct adie_codec_hwsetting_entry handset_lb_rx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = handset_rx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(handset_rx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile lb_handset_rx_profile = {
	.path_type = ADIE_CODEC_RX,
...
/*******************************************************************************
*  handset loopback Tx section
*
********************************************************************************/
static struct adie_codec_action_unit handset_tx_8KHz_osr256_actions[] =
{
	{ ADIE_CODEC_ACTION_STAGE_REACHED, ADIE_CODEC_DIGITAL_OFF },
	{ ADIE_CODEC_ACTION_ENTRY,ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x01)},
	{ ADIE_CODEC_ACTION_ENTRY,ADIE_CODEC_PACK_ENTRY(0x80, 0x01, 0x00) },
	...
	{ ADIE_CODEC_ACTION_ENTRY,	ADIE_CODEC_PACK_ENTRY(0x86, 0xff, 0x00)},
	{ ADIE_CODEC_ACTION_ENTRY,	ADIE_CODEC_PACK_ENTRY(0x8A, 0x50, 0x40)},
	{ ADIE_CODEC_ACTION_ENTRY,	ADIE_CODEC_PACK_ENTRY(0x91, 0xFF, 0x01)}, /* Start loop back */
	{ ADIE_CODEC_ACTION_STAGE_REACHED, ADIE_CODEC_DIGITAL_ANALOG_READY},
	{ ADIE_CODEC_ACTION_ENTRY,	ADIE_CODEC_PACK_ENTRY(0x8A, 0x10, 0x30)},
	{ ADIE_CODEC_ACTION_ENTRY,	ADIE_CODEC_PACK_ENTRY(0x0D, 0xFF, 0x00)},
	{ ADIE_CODEC_ACTION_ENTRY,	ADIE_CODEC_PACK_ENTRY(0x83, 0x14, 0x00)},
...
static struct adie_codec_hwsetting_entry handset_lb_tx_settings[] = {
	{
		.freq_plan = 8000,
		.osr = 256,
		.actions = handset_tx_8KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(handset_tx_8KHz_osr256_actions),
	}
};
static struct adie_codec_dev_profile lb_handset_tx_profile = {
	.path_type = ADIE_CODEC_TX,
...
static int lb_suspend = 0;
static struct adie_codec_path *lb_rx_adie;
static struct adie_codec_path *lb_tx_adie;

static void set_loopback_mode(u32 loop)
{
	int trc;
	...
	if (loop & LB_ENABLE) {
		/* adie config profile for loopback*/
		struct adie_codec_dev_profile *rx_profile;
		struct adie_codec_dev_profile *tx_profile;
		int timeout;

		timeout = 10;
		while (drv->rx_active || drv->tx_active ) {
			msleep(1000);
			if (--timeout < 0)
				return;
		}
		...
		switch (loop & 0xF0)	{
			case LB_PATH_HANDSET :
				rx_profile = &lb_handset_rx_profile;
				tx_profile = &lb_handset_tx_profile;
				break;
			case LB_PATH_HEADSET :
				rx_profile = &lb_headset_rx_profile;
				tx_profile = &lb_headset_tx_profile;
				break;
			case LB_PATH_SPEAKER :
				rx_profile = &lb_handset_rx_profile;
				tx_profile = &lb_speaker_tx_profile;
				break;
			default :
				return;
		}
		...
		/* Configure ADIE */
		adie_codec_open(rx_profile, &lb_rx_adie);
		adie_codec_setpath(lb_rx_adie, 8000, 256);
		...
		adie_codec_proceed_stage(lb_rx_adie,
		ADIE_CODEC_DIGITAL_READY);
		adie_codec_proceed_stage(lb_rx_adie,
		ADIE_CODEC_DIGITAL_ANALOG_READY);			// ?

		/* Set amp for headset path */
		if ((loop & 0xF0) == LB_PATH_HEADSET)
			lge_amp_hp_phone_on();
		...
		if ((loop & 0xF0) == LB_PATH_HANDSET)
			pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_PWM_TCXO);
		else	 if ((loop & 0xF0) == LB_PATH_SPEAKER)
		...
		/* Configure ADIE */
		adie_codec_open(tx_profile, &lb_tx_adie);
		adie_codec_setpath(lb_tx_adie, 8000, 256);
		adie_codec_proceed_stage(lb_tx_adie,
		ADIE_CODEC_DIGITAL_READY);
		adie_codec_proceed_stage(lb_tx_adie,				// ?
		ADIE_CODEC_DIGITAL_ANALOG_READY);
		/* Start AFE for TX */
		...
		adie_codec_proceed_stage(lb_tx_adie,
		ADIE_CODEC_DIGITAL_OFF);
		adie_codec_close(lb_tx_adie);

		/* Power Off amp for headset path */
		if ((lb_status & 0xF0) == LB_PATH_HEADSET)
			lge_amp_off_all();
		...
		MM_INFO("Disable Handset Mic bias\n");
		if ((lb_status & 0xF0) == LB_PATH_HANDSET)
			pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_OFF);
		else	 if ((lb_status & 0xF0) == LB_PATH_SPEAKER)
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* ignore disabling when in disable status */
	if (!LB_IS_ENABLE(lb) && !LB_IS_ENABLE(lb_status))
		return count;

	/* disable loopback when loopback is already enabled*/
	if (LB_IS_ENABLE(lb) && LB_IS_ENABLE(lb_status))
		set_loopback_mode(lb_status & 0xF0);

	/* set loopback */
	set_loopback_mode(lb);
	return count;
}
...
static DEVICE_ATTR(loopback, S_IRUGO|S_IWUGO, show_loopback_value, store_loopback_value);
...
static int snddev_icodec_close(struct msm_snddev_info *dev_info)
{
	...
	if (lb_suspend && drv->rx_active == 0 && drv->tx_active == 0)	{
		...
		MM_INFO("snddev_icodec_close : loopback enabled\n");
		set_loopback_mode(lb_suspend);
		lb_suspend = 0;
	}
#endif
...
{
	extern void set_QTRcal_data(void);
