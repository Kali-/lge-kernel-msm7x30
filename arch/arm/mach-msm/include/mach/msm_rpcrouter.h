...
//#define ABSOLUTE_STRING_LENGTH  40 // ATS_porting ATS_MTC_porting
//#define MAX_STRING_RET (ABSOLUTE_STRING_LENGTH/sizeof(uint8_t)) // ATS_porting ATS_MTC_porting

/* CONFIG_MSM_RPCSERVER_TESTMODE */
struct testmode_relay_result
{
	uint32_t ret_value;
	uint8_t ret_string[MAX_STRING_RET];
};

struct rpc_reply_hdr_testmode
{
	struct rpc_reply_hdr reply;
...
#endif /* CONFIG_MSM_RPCSERVER_TESTMODE [dom_testmode] */
