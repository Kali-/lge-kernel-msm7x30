...
#define CONFIG_MACH_LG_TEST_FRST_SYNC
...
#endif /* defined(CONFIG_LGE_DETECT_PIF_PATCH) */
...
#if defined(CONFIG_MACH_LG_TEST_FRST_SYNC)
static ssize_t
msm_batt_test_frst_sync(struct device *dev, struct device_attribute *attr, char *buf)
{
	lge_test_frst_sync();
	return sprintf(buf, "ste frst_status = 5 and sync\n");
}

static DEVICE_ATTR(test_frst, S_IRUGO, msm_batt_test_frst_sync, NULL);
#endif
