...
#if 0
#define DLOG_ALL(fmt,args...) \
	do { printk(KERN_DEBUG "[%s:%s:%d] "fmt, __FILE__, __func__, __LINE__, \
		    ##args); } \
	while (0)
#else
#define DLOG_ALL(x...) do {} while (0)
#endif
...
static int pmem_free_bitmap(int id, int bitnum)
{
	...
	for (i = 0; i < pmem[id].allocator.bitmap.bitmap_allocs; i++) {
		...
		if (curr_bit == bitnum) {
			...
			DLOG_ALL("bitmap id %d, quanta %d, free %d\n", id, curr_quanta, pmem[id].allocator.bitmap.bitmap_free);
			return 0;
		}
	}
		...
		const unsigned int align)
{
	...
	DLOG_ALL("bitmap id %d, len %ld, align %u\n", id, len, align);
	...
	DLOG_ALL("quantum size %u quanta needed %u free %u id %d\n",
	...
	if (i >= pmem[id].allocator.bitmap.bitmap_allocs) {
		...
		DLOG_ALL("increased # of allocated regions to %d for id %d\n",
			pmem[id].allocator.bitmap.bitmap_allocs, id);
	}

	DLOG_ALL("bitnum %d, bitm_alloc index %d\n", bitnum, i);
