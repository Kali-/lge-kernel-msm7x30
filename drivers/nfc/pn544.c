...
#if defined(CONFIG_LGE_NXP_DRIVER_TEST)
...
#ifdef CONFIG_LGE_NXP_DRIVER_TEST
static int	stReadIntFlag = 0;	//seokmin added for debugging
#endif
#ifdef CONFIG_LGE_MODEL_SU610 //CONFIG_LGE_NXP_NFC sm.cho
		...
		size_t count, loff_t *offset)
{
	...
	if(!stReadIntFlag){ 	 //seokmin added for debugging
		...
		if (!gpio_get_value(pn544_dev->irq_gpio)) {
			...
			pn544_disable_irq(pn544_dev);
			printk("[seokmin]Waiting IRQ[%d]Ends\n", ret);
