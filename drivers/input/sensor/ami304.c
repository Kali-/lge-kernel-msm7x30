...
static ssize_t show_motion_cal_onoff(struct device *dev, 
		struct device_attribute *attr, char *buf)
{
	// ami304mid_data.acccal_result = 0 - success
	// ami304mid_data.acccal_result = 1 - fail
	// ami304mid_data.acccal_result = 2 - not flat place
	// ami304mid_data.acccal_result = 3 - calibrating 
	return sprintf(buf, "%d\n", ami304mid_data.acccal_result);
}

static ssize_t store_motion_cal_onoff(struct device *dev, 
		struct device_attribute *attr, const char *buf, size_t count)
{
	u32    val;
	int na_x, na_y;
	...
	if (val) {
		...
		printk("%s, cal_onoff: %u, na_x: %d, na_y: %d\n", __FUNCTION__,val,  na_x, na_y);
		
		if((na_x > -1000 && na_x < 1000) && (na_y > -1000 && na_y < 1000)) {
			ami304mid_data.controldata[AMI304_CB_ACCCALI] = 1;     // Start-AccCali
			// bluewave96. even accelerometer is "0" status, make it recalibration is work.
			ami304mid_data.controldata[AMI304_CB_ACTIVESENSORS] = 1;
			ami304mid_data.acccal_result = 3;	// calibrating.
		} else {
			ami304mid_data.controldata[AMI304_CB_PD_RESET] = 1;
			ami304mid_data.acccal_result = 2;			// not flat place state.
		}
	} else {
//		ami304mid_data.controldata[AMI304_CB_ACCCALI] = 0;     // Cancel-AccCali
		ami304mid_data.acccal_result = 1;	
	}
	   ...
	   unsigned long arg)
{
	...
	switch (cmd) {
		...
		case AMI304DAE_IOCTL_SET_ACCCAL_RESULT:
			data = (void __user *) arg;
			if (data == NULL)
				break;
			if (copy_from_user(&mode, data, sizeof(mode))) {
			...
			if(mode == 0)
				ami304mid_data.acccal_result = 0;
			else
				ami304mid_data.acccal_result = 1;				
			break;
...
static int __init ami304_init(void)
{
	...
	ami304mid_data.acccal_result = 0; // Acc Cali result
