...
struct so340010_device {
	...
	struct delayed_work 	key_delayed_work ;
...
static struct workqueue_struct *so340010_delayed_wq;
...
static void so340010_key_work_func(struct work_struct *work)
{
	...
	if (state == 0) {
		...
		//printk(KERN_INFO"Key Up(0x%x)\n", state);
		qt_time_diff = jiffies_to_msecs(jiffies) - qt_time_point;
		//printk(KERN_INFO"key diff[%d]!!\n",qt_time_diff);

		qt_time_point = jiffies_to_msecs(jiffies);

		if(qt_time_diff <= 30) {
			//printk(KERN_INFO"flush down event[ox%x]!!!\n", last_key);

			if(last_key != 0) {
				so340010_report_event(0);
				last_key	= 0;
			}
			cancel_delayed_work_sync(&pdev->key_delayed_work);
			flush_workqueue(so340010_delayed_wq);
			return;
		}
	} else {
		qt_time_diff = 0;

		//printk(KERN_INFO"Key Down(0x%x)\n", state);
		qt_time_diff = jiffies_to_msecs(jiffies) - qt_time_point;
		//printk(KERN_INFO"key diff[%d]!!\n",qt_time_diff);

		if(qt_time_diff <= 30) {
			//printk(KERN_INFO"flush up event[ox%x]!!!\n", last_key);
	...
	queue_delayed_work(so340010_delayed_wq, &pdev->key_delayed_work, msecs_to_jiffies(KEY_DELAYED_SEC));
}

static void so340010_delayed_key_work_func(struct work_struct *work)
{
	u16 state;
	int ret;
...
static int so340010_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&so340010_pdev->key_delayed_work, so340010_delayed_key_work_func);
...
static int __init so340010_init(void)
{
	...
	so340010_delayed_wq = create_singlethread_workqueue("so340010_delayed_wq");
	if (!so340010_wq || !so340010_delayed_wq) {
	...
	if (ret < 0) {
		...
		destroy_workqueue(so340010_delayed_wq);
		return ret;
	}
